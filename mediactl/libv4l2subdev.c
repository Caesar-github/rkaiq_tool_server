...
static int set_format(struct media_pad *pad,
  ...
  media_dbg(pad->entity->media, "Setting up format %s %ux%u on pad %s/%u\n",
            ...
            format->height, pad->entity->info.name, pad->index);
...
static int set_selection(struct media_pad *pad, unsigned int target,
  ...
  ret = v4l2_subdev_set_selection(pad->entity, rect, pad->index, target,
                                  V4L2_SUBDEV_FORMAT_ACTIVE);
  if (ret < 0) {
...
static int set_frame_interval(struct media_pad *pad,
  ...
  media_dbg(pad->entity->media,
            ...
            interval->numerator, interval->denominator, pad->entity->info.name,
            pad->index);

  ret = v4l2_subdev_set_frame_interval(pad->entity, interval, pad->index);
  if (ret < 0) {
    media_dbg(pad->entity->media, "Unable to set frame interval: %s (%d)",
              strerror(-ret), ret);
    return ret;
  }
...
static int v4l2_subdev_parse_setup_format(struct media_device *media,
  ...
  pad = v4l2_subdev_parse_pad_format(media, &format, &crop, &compose, &interval,
                                     p, &end);
  if (pad == NULL) {
