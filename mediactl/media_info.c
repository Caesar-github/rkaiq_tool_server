...
void print_flags(const struct flag_name *flag_names, unsigned int num_entries,
                 __u32 flags) {
  bool first = true;
  unsigned int i;
...
void v4l2_subdev_print_format(struct media_entity *entity, unsigned int pad,
  ...
  printf("\t\t[fmt:%s/%ux%u", v4l2_subdev_pixelcode_to_string(format.code),
  ...
  ret = v4l2_subdev_get_selection(entity, &rect, pad, V4L2_SEL_TGT_CROP_BOUNDS,
                                  which);
  if (ret == 0)
    printf("\n\t\t crop.bounds:(%u,%u)/%ux%u", rect.left, rect.top, rect.width,
           rect.height);

  ret = v4l2_subdev_get_selection(entity, &rect, pad, V4L2_SEL_TGT_CROP, which);
  if (ret == 0)
    printf("\n\t\t crop:(%u,%u)/%ux%u", rect.left, rect.top, rect.width,
           rect.height);

  ret = v4l2_subdev_get_selection(entity, &rect, pad,
                                  V4L2_SEL_TGT_COMPOSE_BOUNDS, which);
  if (ret == 0)
    printf("\n\t\t compose.bounds:(%u,%u)/%ux%u", rect.left, rect.top,
           rect.width, rect.height);

  ret = v4l2_subdev_get_selection(entity, &rect, pad, V4L2_SEL_TGT_COMPOSE,
                                  which);
  if (ret == 0)
    printf("\n\t\t compose:(%u,%u)/%ux%u", rect.left, rect.top, rect.width,
           rect.height);
...
static const struct flag_name bt_standards[] = {
    {V4L2_DV_BT_STD_CEA861, "CEA-861"}, {V4L2_DV_BT_STD_DMT, "DMT"},
...
void v4l2_subdev_print_dv_timings(const struct v4l2_dv_timings *timings,
                                  const char *name) {
  ...
  case V4L2_DV_BT_656_1120: {
    ...
    printf(" %ux%u%s%llu (%ux%u)", bt->width, bt->height,
           bt->interlaced ? "i" : "p",
...
void v4l2_subdev_print_pad_dv(struct media_entity *entity, unsigned int pad,
                              enum v4l2_subdev_format_whence which) {
  ...
  switch (caps.type) {
  case V4L2_DV_BT_656_1120:
    printf(" min:%ux%u@%llu max:%ux%u@%llu", caps.bt.min_width,
           caps.bt.min_height, caps.bt.min_pixelclock, caps.bt.max_width,
           caps.bt.max_height, caps.bt.max_pixelclock);
...
const char *media_entity_type_to_string(unsigned type) {
  static const struct {
    ...
    const char *name;
      ...
      {MEDIA_ENT_T_DEVNODE, "Node"}, {MEDIA_ENT_T_V4L2_SUBDEV, "V4L2 subdev"},
  };
...
const char *media_pad_type_to_string(unsigned flag) {
  static const struct {
    ...
    const char *name;
      ...
      {MEDIA_PAD_FL_SINK, "Sink"}, {MEDIA_PAD_FL_SOURCE, "Source"},
  };
...
void media_print_pad_text(struct media_entity *entity,
